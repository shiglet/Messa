...
namespace Messa.Game.BidHouse
{
    public class BidHouse : IBidHouse
    {
        ...
        public async Task<bool> StartBidHouseDialog(NpcActionId ActionId)
        {
            ...
            _account.Logger.Log($"Ouverture de la fenêtre HDV du PNJ {npcName}({npcMapId})", LogMessageType.Info);
        ...
        public async Task<bool> BuyItem(uint itemUId, uint quantity)
        {
            ...
            _account.Logger.Log($"Achat de {quantity} de l'item portant l'uid {itemUId} au prix de {Price}",
                LogMessageType.Info);
            /* All good, we pay */
