...
using Messa.API.Gamedata.D2p.Elements;
...
using Messa.API.Protocol.Network.Messages.Game.Context.Roleplay.Fight;
...
using Messa.Core;
using Messa.Game.Entity;
using Messa.Game.Map.Elements;
//using DofusMapControl;
using IMap = Messa.API.Game.Map.IMap;
using Npc = Messa.Game.Entity.Npc;

namespace Messa.Game.Map
{
    ...
    public class Map : IMap
    {
        ...
        {
            _account.Logger.Log("Appel de fonction fight!");
        }
        ...
        /*private void MapControl_CellClicked(MapControl control, MapCell cell, MouseButtons buttons, bool hold)
        {
            ...
            {
                if (e.Sucess)
                    _account.Logger.Log($"Déplacement réussi sur la cell: {cell.Id}");
            };
            mov.PerformMovement();
