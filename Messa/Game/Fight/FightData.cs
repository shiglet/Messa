...
using Messa.API.Game.World.Pathfinding;
...
using Messa.API.Protocol.Network.Messages.Game.Actions;
using Messa.API.Protocol.Network.Messages.Game.Actions.Fight;
using Messa.API.Protocol.Network.Messages.Game.Actions.Sequence;
...
using Messa.API.Protocol.Network.Messages.Game.Context.Fight.Character;
using Messa.API.Protocol.Network.Types.Game.Actions.Fight;
using Messa.API.Protocol.Network.Types.Game.Context.Fight;
using Messa.API.Utils;
using Messa.API.Utils.Enums;
using Messa.Game.Fight.Fighters;
using Companion = Messa.Game.Fight.Fighters.Companion;
using Monster = Messa.Game.Fight.Fighters.Monster;
...
namespace Messa.Game.Fight
{
    public class FightData : IFightData
    {
        ...
        private void HandleGameFightStartingMessage(IAccount account, GameFightStartingMessage message)
        {
            FightStarted?.Invoke();
            Account.Character.Status = CharacterStatus.Fighting;
            Account.Logger.Log("Début du combat");
        }
        ...
        private void HandleGameActionFightDeathMessage(IAccount account, GameActionFightDeathMessage message)
        {
            ...
            if (Fighter.Id == message.TargetId)
                Account.Logger.Log("Bot mort");
            if (Monsters.Exists(m => m.Id == message.TargetId))
                Account.Logger.Log($"Monstre est mort");
