...
using Messa.API.Protocol.Network.Messages.Game.Character.Creation;
using Messa.API.Protocol.Network.Messages.Game.Character.Deletion;
using Messa.API.Protocol.Network.Messages.Game.Character.Stats;
using Messa.API.Protocol.Network.Messages.Game.Chat.Channel;
using Messa.API.Protocol.Network.Messages.Game.Context;
using Messa.API.Protocol.Network.Messages.Game.Context.Roleplay;
using Messa.API.Protocol.Network.Messages.Game.Context.Roleplay.Quest;
using Messa.API.Protocol.Network.Messages.Game.Friend;
using Messa.API.Protocol.Network.Messages.Game.Initialization;
    ...
    public class Character : ICharacter
    {
        ...
        private void HandleBasicCharactersListMessage(IAccount account, BasicCharactersListMessage message)
        {
            ...
            else
            {
                ...
                account.Logger.Log($"Connection sur le personnage {c.Name}");
        ...
        private void HandleCharactersListMessage(IAccount account, CharactersListMessage message)
        {
            if (message.Characters.Count == 0)
            {
                account.Logger.Log("Pas de personnage.");
                account.Logger.Log("Création du personnage en cours.");
                account.Network.SendToServer(new CharacterNameSuggestionRequestMessage());
            }
            else
            {
                var c = message.Characters[0];
                account.Logger.Log("Connexion sur le personnage " + c.Name);
        ...
        private void HandleCharacterCreationResultMessage(IAccount account, CharacterCreationResultMessage message)
        {
            switch ((CharacterCreationResultEnum)message.Result)
            {
                ...
                case CharacterCreationResultEnum.ERR_INVALID_NAME:
                    account.Logger.Log("Ce nom de personnage est invalide.", LogMessageType.Public);
                    break;

                case CharacterCreationResultEnum.ERR_NAME_ALREADY_EXISTS:
                    account.Logger.Log("Ce nom de personnage est déjà pris.", LogMessageType.Public);
                    break;

                case CharacterCreationResultEnum.ERR_TOO_MANY_CHARACTERS:
                    account.Logger.Log("Vous avez déjà atteint la limite de personnages disponible.",
        ...
        {
            account.Logger.Log($"{message.Reason}", LogMessageType.Public);
            account.Network.SendToServer(new CharacterNameSuggestionRequestMessage());
        }
        ...
        {
            account.Logger.Log($"Pseudo Suggérer: {message.Suggestion}");
        ...
        private void HandleCharacterExperienceGainMessage(IAccount account, CharacterExperienceGainMessage message)
        {
            if (message.ExperienceCharacter != 0)
            {
                ...
                account.Logger.Log($"Vous avez gagné {message.ExperienceCharacter} points d'expérience.",
                    LogMessageType.Info);
            }
            if (message.ExperienceGuild != 0)
                account.Logger.Log($"Votre guilde a gagné {message.ExperienceGuild} points d'expérience.",
                    LogMessageType.Info);
            if (message.ExperienceMount != 0)
                account.Logger.Log($"Vous monture a gagné {message.ExperienceMount} points d'expérience.",
                    LogMessageType.Info);
        }
        ...
        {
            account.Logger.Log($"{message.Name} viens de passer niveau {message.NewLevel}.", LogMessageType.Info);
        }

        private void HandleCharacterLevelUpMessage(IAccount account, CharacterLevelUpMessage message)
        {
            account.Logger.Log($"Vous venez de passer niveau {message.NewLevel}.", LogMessageType.Info);
        ...
        private void HandleLifePointsRegenEndMessage(IAccount account, LifePointsRegenEndMessage message)
        {
            ...
            if (message.LifePointsGained != 0)
                account.Logger.Log($"Vous avez récupéré {message.LifePointsGained} points de vie.",
                    LogMessageType.Info);
        }
        ...
        {
            account.Logger.Log("Connection Event Type: " + message.EventType + " | MessageID: " + message.MessageID,
